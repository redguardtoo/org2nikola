...
Put focus in anywhere of subtree "article 2" and =M-x org2nikola-export-subtree=. That's it.

Org2nikola output files into "~/.config/nikola". You need run =cd ~/.config/nikola;nikola build= to render website.

Please note =M-x org2nikola-export-subtree=only creates HTML file for current node. You may need =M-x org2nikola-rerender-published-posts= at least once to re-create HTML files for old posts before "nikola build".
